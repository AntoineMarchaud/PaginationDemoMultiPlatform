package com.amarchaud.shared.data.db
...
object SqlDelightConverter {
    ...
    data object LocalDateRoomConverter {
        ...
        fun localDateToString(localDate: LocalDateTime): String { // call when insert data in db
            return localDate.toString()  // LocalDateTime is automatically formatted as ISO-8601
        }

        fun stringToLocalDate(str: String?): LocalDateTime? { // call when loading db
            val iso8601Text = str?.dropLast(1) // ISO-8601 format

            return iso8601Text?.let { LocalDateTime.parse(it) }  // Parse using ISO-8601 format        }
        }
    }
}
...
class PaginationDemoDaoImpl(
    ...
    override fun getUsersPagingSource(): PagingSource<Int, UsersEntity> {
        return QueryPagingSource(
            ...
            initialOffset = 0,
        ) as androidx.paging.PagingSource<Int, UsersEntity>
    }
